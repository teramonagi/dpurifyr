...
test_that("encode_onehot results agree with one-hot encoding by hand", {
  df <- data.frame(x=c('a', 'b', 'a', 'c'), hoge=1:4)
  pp <- dpurifyr::encode_onehot(df, x, drop_first=FALSE)
...
test_that("encode_onehot using two variables with different separator should be also possible", {
  df <- data.frame(x=c('a', 'b', 'a', 'c'), hoge=c(1,1,2,2))
  pp <- dpurifyr::encode_onehot(df, x, hoge, drop_first=FALSE, sep="-")
